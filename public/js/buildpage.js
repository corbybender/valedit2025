...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const formsSelector = document.getElementById("forms-selector");
  ...
  function initializeDragAndDrop() {
    if (typeof Sortable === "undefined") {
      ...
      setTimeout(() => {
        if (typeof Sortable === "undefined") {
          logger.error(
            "❌ Sortable library failed to load! CDN may be unreachable."
          );
          console.error(
            "❌ Sortable library is not loaded! Check if SortableJS CDN is accessible."
          );
          console.error(
          ...
          if (typeof showNotification === "function") {
            showNotification(
              "Drag and drop functionality unavailable. Please refresh the page.",
              "error",
              { timeout: 10000 }
            );
          } else {
  ...
  if (formsSelector)
    formsSelector.addEventListener("change", handleFormSelection);
  ...
  setTimeout(() => {
    ...
    if (typeof showNotification === "undefined") {
      logger.debug(
        "🔔 FALLBACK: Creating comprehensive notification system..."
      );
  ...
  function initializeSharedBlocksDraggable() {
    if (typeof Sortable === "undefined") {
      logger.error(
        "❌ Sortable library not available for shared blocks initialization"
      );
      return;
    }
  ...
  async function fetchAndPopulateForms() {
    if (!currentWorkingSite || !currentWorkingSite.CurrentWorkingSite) {
      logger.debug("No working site selected, skipping forms fetch");
      return;
    }

    const websiteId = currentWorkingSite.CurrentWorkingSite;
    logger.debug("🔄 Fetching published forms for website:", websiteId);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("❌ API Error response:", errorText);
        throw new Error(
          `Failed to fetch forms: ${response.status} - ${errorText}`
        );
      }

      const result = await response.json();
      logger.debug("📦 Forms API result:", result);

      if (!formsSelector) {
        logger.error("❌ Forms selector not found in DOM");
        return;
      }

      // Clear existing options
      formsSelector.innerHTML = '<option value="">Select a form...</option>';

      if (!result.success || !result.forms || result.forms.length === 0) {
        formsSelector.innerHTML =
          '<option value="">No published forms available</option>';
        logger.debug("📝 No published forms found for website:", websiteId);
        return;
      }

      // Filter only published forms
      const publishedForms = result.forms.filter((form) => form.IsPublished);
      ...
      publishedForms.forEach((form) => {
        ...
        formsSelector.appendChild(option);
      });

      // Store forms data for later use
      window.formsData = publishedForms;
      logger.info(
        `✅ Successfully loaded ${publishedForms.length} published forms`
      );
    } catch (error) {
      console.error("❌ Error fetching forms:", error);
      formsSelector.innerHTML = '<option value="">Error loading forms</option>';
    }
  }

  function handleFormSelection(e) {
    const selectedFormId = e.target.value;
    logger.info("📋 Selected form ID:", selectedFormId);
    ...
    if (!selectedFormId || !window.formsData) {
      logger.debug("📝 No form selected or no forms data available");
      return;
    }

    // Find the selected form
    const selectedForm = window.formsData.find(
      (form) => form.FormID == selectedFormId
    );

    if (!selectedForm) {
      logger.error("❌ Selected form not found in forms data");
      return;
    }

    logger.info("📦 Creating draggable form block for:", selectedForm);

    // Create draggable form block
    const formElement = document.createElement("div");
    formElement.className = "form-block";
    formElement.draggable = true;
    formElement.dataset.formId = selectedForm.FormID;
    formElement.dataset.formName = selectedForm.Name;
    formElement.dataset.blockType = "form";
    ...
    formElement.innerHTML = `
      ...
      <div class="form-block-content">
        <div class="form-block-name">${selectedForm.Name}</div>
        <div class="form-block-description">${
          selectedForm.Description || "Form ready to be added to page"
        }</div>
      </div>
    `;
    ...
    // Initialize Sortable for the new form block
    initializeFormsDraggable();
  }

  function initializeFormsDraggable() {
    if (typeof Sortable === "undefined") {
      logger.error(
        "❌ Sortable library not available for forms initialization"
      );
      return;
    }

    if (formsContainer) {
      logger.debug("✅ Initializing forms-container sortable");
      new Sortable(formsContainer, {
        group: { name: "shared", pull: "clone", put: false },
        sort: false,
        animation: 150,
      });
    } else {
      logger.warn("⚠️ formsContainer not found");
    }
  }
...
`
  ...
  async function loadPageLayout(layoutId) {
    ...
    try {
      ...
      placeholders.forEach((placeholder, index) => {
        ...
        if (typeof Sortable === "undefined") {
          logger.error(
            "❌ Sortable library not available for placeholder initialization"
          );
          return;
        }
        ...
        logger.debug(
          `✅ Sortable initialized for placeholder: ${placeholder.id}`
        );
      });
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          // Handle form blocks
          logger.info(
            "🚀 FORM BLOCK DETECTED. Preparing to create form block..."
          );

          const formId = originalCard.dataset.formId;
          const formName = originalCard.dataset.formName;

          logger.info("📦 Form details:", { formId, formName });

          // Create form HTML content
          const formHtml = `
            <div class="form-container" data-form-id="${formId}">
              <form action="/content/forms/submit/${formId}" method="POST" class="dynamic-form">
                <div class="form-loading">
                  <p>Loading form: ${formName}...</p>
                </div>
              </form>
            </div>
          `;

          // Create a payload for the form block (similar to empty blocks)
          const payload = {
            pageId: currentPageId,
            contentTemplateId: null, // No template for form block
            placeholderId: placeholder.id,
            sortOrder: placeholder.children.length,
            blockType: "form",
            formId: formId,
          ...
          logger.info(
            "📦 Payload for form block:",
            JSON.stringify(payload, null, 2)
          );

          const formResponse = await fetch("/api/pagecontentblocks/page", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
          ...
          logger.info(
            "📬 Server response status for form block:",
            formResponse.status
          );
          ...
          if (!formResponse.ok) {
            ...
            console.error("❌ Server error response (text):", errorText);
            const formErrorData = JSON.parse(errorText);
            throw new Error(
              formErrorData.error || "Failed to create form block."
            );
          }

          const formBlockData = await formResponse.json();
          logger.info(
            "✅ Successfully created form block. Server data:",
            formBlockData
          );

          // Create the form block instance
          createFormBlockInstance(tempElement, formBlockData, formId, formName);
        } else {
  ...
  function createFormBlockInstance(
    elementToReplace,
    blockData,
    formId,
    formName
  ) {
    logger.info("🚀 Creating form block instance:", {
      blockData,
      formId,
      formName,
    });
    ...
    const blockWrapper = document.createElement("div");
    blockWrapper.className = "content-block-instance form-block-instance";
    blockWrapper.dataset.instanceId = blockData.ID;
    blockWrapper.dataset.formId = formId;
    blockWrapper.dataset.blockType = "form";

    const header = document.createElement("div");
    header.className = "content-block-header";
    header.innerHTML = `
      <div class="content-block-title">Form: ${formName}</div>
      <div class="content-block-actions">
        <button class="edit-btn">Edit Form</button>
    ...
    if (elementToReplace && elementToReplace.parentNode) {
      elementToReplace.parentNode.replaceChild(blockWrapper, elementToReplace);
      logger.info("✅ Form block replaced temp element");
    } else {
      canvasArea.appendChild(blockWrapper);
      logger.info("✅ Form block appended to canvas");
    }

    // Create initial form content
    const formContent = `
      <div class="form-container" style="padding: 20px; border: 1px solid #ddd; border-radius: 4px;">
        <h3 style="margin-top: 0; color: #007bff;">📝 ${formName}</h3>
        <div class="form-loading" style="text-align: center; color: #6c757d;">
          <p><i class="fas fa-spinner fa-spin"></i> Loading form...</p>
          <p style="font-size: 12px;">Form ID: ${formId}</p>
        </div>
      </div>
    `;
    ...
    const iframeContent = `<!DOCTYPE html>
      ...
      <head>
        <style>
          ...
          .form-loading { text-align: center; color: #6c757d; }
        </style>
      </head>
    ...
    // Load the actual form content asynchronously
    loadFormContent(iframe, formId);

    logger.info("✅ Form block instance created successfully");
  }

  async function loadFormContent(iframe, formId) {
    try {
      logger.info("🔄 Loading form content for form ID:", formId);
      ...
      const formElements = form.FormElements || { elements: [] };

      logger.info("📦 Form data loaded:", { form, formElements });

      // Generate form HTML
      let formHtml = `
        <div class="form-container" style="padding: 20px; border: 1px solid #ddd; border-radius: 4px;">
          <h3 style="margin-top: 0; color: #007bff;">📝 ${form.Name}</h3>
          ${
            form.Description
              ? `<p style="color: #6c757d; margin-bottom: 20px;">${form.Description}</p>`
              : ""
          }
          <form action="/content/forms/submit/${formId}" method="POST" class="dynamic-form">
      `;
      ...
      // Add form elements
      if (formElements.elements && formElements.elements.length > 0) {
        formElements.elements.forEach((element, index) => {
          formHtml += generateFormElementHtml(element, index);
        });
      } else {
        formHtml +=
          '<p style="color: #6c757d; font-style: italic;">No form elements configured.</p>';
      }
      ...
      logger.error("❌ Error loading form content:", error);

      const errorContent = `
        <div class="form-container" style="padding: 20px; border: 1px solid #dc3545; border-radius: 4px; background: #f8d7da;">
          <h3 style="margin-top: 0; color: #dc3545;">❌ Error Loading Form</h3>
          <p style="color: #721c24;">Could not load form content: ${error.message}</p>
          <p style="font-size: 12px; color: #721c24;">Form ID: ${formId}</p>
        </div>
      `;

      const iframeContent = `<!DOCTYPE html>
        <html>
        <head><style>body { margin: 0; font-family: Arial, sans-serif; }</style></head>
  ...
  function generateFormElementHtml(element, index) {
    const fieldName = `field_${index}`;
    let html = `<div class="form-group">`;

    if (element.label) {
      html += `<label for="${fieldName}">${element.label}${
        element.required ? " *" : ""
      }</label>`;
    }
    ...
    switch (element.type) {
      ...
      case "url":
        ...
        break;
      case "textarea":
        html += `<textarea id="${fieldName}" name="${fieldName}" rows="4" ${
          element.required ? "required" : ""
        } ${
          element.placeholder ? `placeholder="${element.placeholder}"` : ""
        }></textarea>`;
          ...
          element.options.forEach((option, optIndex) => {
            html += `
              <div class="checkbox-item">
                <input type="checkbox" id="${fieldName}_${optIndex}" name="${fieldName}[]" value="${
              option.value || option.label
            }">
                <label for="${fieldName}_${optIndex}">${option.label}</label>
              </div>
            `;
          });
        }
          ...
          element.options.forEach((option, optIndex) => {
            html += `
              <div class="radio-item">
                <input type="radio" id="${fieldName}_${optIndex}" name="${fieldName}" value="${
              option.value || option.label
            }" ${element.required ? "required" : ""}>
                <label for="${fieldName}_${optIndex}">${option.label}</label>
              </div>
  ...
  async function handleCanvasClick(e) {
    ...
    if (e.target.classList.contains("edit-btn")) {
      ...
      } else if (blockInstance.classList.contains("form-block-instance")) {
        // Handle form block editing
        const formId = blockInstance.dataset.formId;
        logger.info(
          "📝 FORM BLOCK - Redirecting to forms management for form:",
          formId
        );

        if (formId) {
          // Redirect to forms management page with the specific form
          window.open(`/content/forms?edit=${formId}`, "_blank");
          showNotification("Opening form editor in new tab", "info");
        } else {
          showNotification("Error: Form ID not found", "error");
        }
        return; // Exit early to prevent regular block logic
    ...
    allCards.forEach((card) => {
      ...
      const categoryMatch =
        ...
        (selectedCategory === "null" &&
          (!card.dataset.categoryId || card.dataset.categoryId === "null")) ||
  ...
  async function loadExistingPage(pageId) {
    ...
    try {
      ...
      // Fetch forms data before loading blocks (needed for form block reconstruction)
      await fetchAndPopulateForms();
          ...
          if (placeholder) {
            try {
              ...
              if (response.ok) {
                ...
                // Check if this is a special block type (JavaScript, CSS, Empty, Form)
  ...
  async function initializePage() {
    ...
    try {
      ...
      // Initialize shared blocks and forms
      if (currentWebsite && currentWebsite.CurrentWorkingSite) {
        logger.info("🔄 Loading shared blocks and forms...");
  ...
  function populateAlternativeUrls(urls) {
    ...
    urls.forEach((url, index) => {
      ...
      urlDiv.innerHTML = `
        ...
        />
        <button type="button" class="remove-url-btn" ${
          index === 0 && urls.length === 1 ? "disabled" : ""
        }>
          <i class="fas fa-trash"></i>
        </button>
      `;
  ...
  async function handleSaveMetadata() {
    ...
    try {
      if (!currentPageId) {
        showNotification(
          "Please create a page first before saving metadata",
          "error"
        );
        return;
      }
      ...
      const metaTitle =
        document.getElementById("meta-title")?.value?.trim() || "";
      const metaKeywords =
        document.getElementById("meta-keywords")?.value?.trim() || "";
