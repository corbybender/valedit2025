...
router.get("/", async (req, res) => {
  try {
    ...
    logger.debug(`📋 Logs route - authorID: ${authorID}`);
...
router.get("/api", async (req, res) => {
  try {
    ...
    console.log(
      `📋 DEBUG: Loading logs - page: ${page}, limit: ${limit}, level: ${level}, search: "${search}"`
    );
    ...
    try {
      if (fs.existsSync(logFilePath)) {
        ...
        console.log(
          `📋 DEBUG: Log file found, size: ${logContent.length} characters`
        );
      } else {
    ...
    if (logContent) {
      ...
      for (const line of logLines) {
        ...
        if (match) {
          ...
          if (currentEntry.Message.length < 500) {
    ...
    console.log(
      `📋 DEBUG: Returning ${paginatedLogs.length} logs out of ${total} total`
    );
...
router.delete("/cleanup", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to cleanup logs", details: err.message });
  }
});
